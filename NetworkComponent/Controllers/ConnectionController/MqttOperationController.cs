...
namespace NetworkComponent.Controllers
{
    ...
    public class MqttOperationController : ControllerBase
    {
        ...
        /// <summary>
        /// 【获取托管订阅收到的消息】用于 Web 控制台轮询展示订阅主题 / 答复主题收到的消息。
        /// 需要先通过 SubTopic 订阅；PublishAndWaitReply 会自动订阅答复主题。
        /// 请求方式：GET
        /// </summary>
        /// <param name="clientId">MQTT客户端标识ID</param>
        /// <param name="topic">已订阅的主题</param>
        /// <returns>最近收到的消息列表（新的在前）</returns>
        [HttpGet]
        public async Task<ApiUnifiedReturnStructure<List<ConnectionMqtt.LocalEntity.ReceivedMqttMessage>>> GetReceivedMessages(
            string clientId, string topic)
        {
            var list = await _mqttService.GetReceivedMessagesAsync(clientId, topic);
